...
func TestTracker(t *testing.T) {
	close := testserver.StartUDPServer()
	transport := udptracker.NewUDPTransport()
	go transport.Run()
	defer close()
	defer transport.Close()

	announce, _ := url.Parse("udp://localhost:8001/announce")
	tr := udptracker.New(*announce, transport)
